...
final class OpenAIService {
    ...
    // Get API key from Info.plist (set via Secrets.xcconfig)
    private var infoPlistKey: String {
        Bundle.main.object(forInfoDictionaryKey: "OPENAI_API_KEY") as? String ?? ""
    }

    // Get API key with priority: stored (from backend) > Info.plist (from xcconfig)
    private var apiKey: String {
        ...
        // Fallback to Info.plist key (from Secrets.xcconfig)
        return infoPlistKey
    }
