        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 32) {
                    ...
                    // Status Text
                    VStack(spacing: 8) {
                        Text(isDetoxActive ? L10n.Detox.stayFocused : L10n.Detox.digitalDetox)
                            .font(.system(size: 24, weight: .bold))
                            .foregroundStyle(isDark ? .white : .black)

                        Text(isDetoxActive ? L10n.Detox.allAppsBlocked : L10n.Detox.takeBreak)
                            .font(.system(size: 16))
            ...
            .navigationTitle(L10n.Detox.title)
        }
    }
        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: isDetoxActive ? "stop.fill" : "play.fill")
                    .font(.system(size: 18))
                Text(isDetoxActive ? L10n.Detox.endDetox : L10n.Detox.startDetox)
...
struct DurationButton: View {
    ...
    private var formatDuration: String {
        if minutes < 60 {
            return L10n.Detox.minutes(minutes)
        } else {
            return L10n.Detox.hours(minutes / 60)
        }
    }
}
